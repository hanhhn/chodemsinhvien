...
class ModelToolSeoPackageEditor extends Model {
	...
	public function simple ( $request, $type, $lang, $columns )
	{
      ...
      if ($this->config->get('mlseo_ml_mode')) {
        // disabled because of too much performance requiring
        //$extra_select = ",(SELECT keyword FROM " . DB_PREFIX . "url_alias WHERE query = CONCAT('".$type."_id=' , ".$type."_id) AND (language_id = '".(int) $lang."' OR language_id = 0) LIMIT 1) AS seo_keyword";
      } else {
        // disabled because of too much performance requiring
